...
export default function UploadPage() {
  ...
  useEffect(() => {
    Promise.resolve().then(() => loadUploads());
  }, [loadUploads]);
  ...
  const handleFiles = useCallback(
    ...
    [account, loadUploads, loadAccounts]
  );

  const canUpload = account.trim().length > 0;
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-8 px-4 py-8 sm:px-6 sm:py-12">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-600 dark:text-violet-200/60">
          ...
          already imported from that same account.
        </p>
      </div>

      <div className="flex flex-col gap-2 rounded-3xl border border-violet-100 bg-white p-4 shadow-[0_2px_20px_-6px_rgba(139,92,246,0.12)] dark:border-white/10 dark:bg-white/5 dark:shadow-none">
        ...
        <p className="text-xs text-zinc-500 dark:text-violet-200/50">
          ...
          together in one go all get the same account — upload each bank&apos;s statement
          separately.
        </p>
      </div>
      ...
      <div
        ...
        className={`flex flex-col items-center justify-center gap-3 rounded-3xl border-2 border-dashed px-6 py-16 text-center transition-colors ${
          !canUpload
            ? "cursor-not-allowed border-violet-100 bg-violet-50/40 opacity-60 dark:border-white/5 dark:bg-white/5"
            : dragging
              ? "cursor-pointer border-violet-400 bg-violet-50 dark:border-violet-500 dark:bg-white/10"
              : "cursor-pointer border-violet-200 bg-white hover:bg-violet-50/50 dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10"
        }`}
      >
        <input
          ref={inputRef}
          ...
          {outcomes.map((o, i) => (
            ...
            >
              <div className="font-medium">
                {o.filename} <span className="font-normal opacity-70">· {o.account}</span>
              </div>
              {o.error ? (
                <div>{o.error}</div>
              ) : (
              ...
              <tbody>
                {uploads.map((u) => (
                  <tr key={u.id} className="border-t border-violet-100 dark:border-white/10">
                    ...
                    <td className="max-w-[120px] truncate px-4 py-2 text-zinc-500 dark:text-violet-200/50">
                      {u.account || "—"}
                    </td>
